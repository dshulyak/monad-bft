...
async fn run(node_state: NodeState) -> Result<(), ()> {
    ...
    if let Some(metrics_config) = &node_state.metrics {
        tokio::spawn({
            let addr = metrics_config.addr.clone();
            ...
            async move {
                ...
                let server = match start_metrics_server(addr, server_state) {
                    Ok(server) => server,
                    Err(err) => {
        ...
        tokio::select! {
            ...
            event = executor.next().instrument(ledger_span.clone()) => {
                ...
                let commands = {
                    ...
                    prometheus_metrics.record_state_metrics(state.metrics());
