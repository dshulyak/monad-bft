...
use monad_fair_queue::{FairQueue, FairQueueBuilder, Identity, IdentityScore};
...
impl IdentityScore for TestScorer {
    ...
    fn score(&self, identity: &Self::Identity) -> f64 {
        self.scores.get(identity).copied().unwrap_or(1.0)
    }
}
            ...
            |b, &num_ids| {
                ...
                b.iter(|| {
                    ...
                    for identity in 0..num_ids {
                        let _ = queue.push(
                            black_box(Identity::Authenticated(identity)),
                            black_box(identity),
                        );
                    }
            ...
            |b, &num_ids| {
                ...
                b.iter_batched(
                    || {
                        let mut queue = make_queue(scorer.clone());
                        for identity in 0..num_ids {
                            let _ = queue.push(Identity::Authenticated(identity), identity);
                        }
                        queue
                    },
                    |mut queue| {
